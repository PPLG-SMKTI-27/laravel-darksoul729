        ...
        <div className="min-h-screen flex flex-col font-sans relative overflow-x-hidden text-slate-800">
            ...
            <nav className="relative z-50 pt-8 px-6 md:px-12 flex items-center justify-between h-20">
                ...
                <div className="w-12 h-12 md:w-16 md:h-16 flex-shrink-0 z-10">
                    <div className="w-full h-full bg-gradient-to-br from-pink-400 to-pink-500 rounded-2xl shadow-[0_6px_0_#db2777,0_10px_20px_rgba(236,72,153,0.3)] border-2 border-pink-300 flex items-center justify-center transform hover:scale-105 transition-transform cursor-pointer" onClick={() => window.location.href = '/'}>
                        {/* Glossy Reflection */}
                        <div className="absolute top-1 left-2 right-2 h-1/3 bg-gradient-to-b from-white/40 to-transparent rounded-t-xl"></div>
                        <div className="w-1/2 h-1/2 bg-white/20 rounded-full backdrop-blur-sm shadow-inner"></div>
                    </div>
                </div>
                ...
                <div className="hidden md:flex absolute left-1/2 top-8 -translate-x-1/2 bg-gradient-to-b from-[#60a5fa] to-[#3b82f6] p-2 rounded-full shadow-[0_8px_0_#1d4ed8,0_15px_30px_rgba(37,99,235,0.3)] border-[3px] border-blue-300 items-center justify-center gap-1 z-10">
                    ...
                    {navItems.map((item) => {
                        const isActive = page === item.name || (page === 'LandingPage' && item.name === 'Home') || (window.location.pathname === item.href);
                        return (
                            <a
                                key={item.name}
                                href={item.href}
                                className={`
                                    relative px-5 py-2 rounded-full font-black text-xs md:text-sm tracking-wide transition-all duration-200
                                    ${isActive
            ...
            <div className="md:hidden absolute top-8 right-6 z-[60]">
                <button
                    ...
                    className="
                        ...
                        bg-yellow-400 
                        rounded-2xl 
                        shadow-[0_6px_0_#ca8a04,0_10px_10px_rgba(0,0,0,0.2)] 
                        border-b-[4px] border-r-[4px] border-yellow-300 
