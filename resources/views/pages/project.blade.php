        ...
        <section class="py-16 border-y border-white/20 mb-20">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                @foreach($repos as $index => $repo)
                    <div class="group relative bg-[#0a0a0a] border border-gray-800 flex flex-col h-full hover:border-white transition-colors duration-300 overflow-hidden">
                        @if($repo->image)
                        ...
                        <div class="flex items-center gap-3 mb-6 border-b border-gray-800 pb-4">
                            ...
                            <span class="font-mono text-[10px] text-gray-500 uppercase tracking-widest">PROJECT</span>
                            <span class="ml-auto font-mono text-[10px] text-gray-600">
                                {{ $repo->updated_at->format('M Y') }}
                            </span>
                        </div>

                        <h3 class="text-xl font-display font-bold mb-3 uppercase leading-none text-gray-300 group-hover:text-white transition">
                            <a href="{{ $repo->link ?? '#' }}" class="hover:underline decoration-1 underline-offset-4">{{ $repo->title }}</a>
                        </h3>
                        ...
                        <div class="mt-auto flex justify-between items-center border-t border-gray-800 pt-4">
                            <span class="font-mono text-[10px] text-gray-600">STATUS: {{ strtoupper($repo->status) }}</span>
                            <a href="{{ $repo->link ?? '#' }}" class="text-xs font-mono uppercase text-white hover:text-gray-400 transition">View -></a>
                        </div>
                    </div>
                @endforeach
            </div>
